...
server.post("/mediaItems/", function (req, res, next) {
  const error = validateMediaItem(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new media items.
    next();
  }
});

server.post("/directors/", function (req, res, next) {
  const error = validateDirector(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.name); // Generate a slug for new directors.
    next();
  }
});
...
function validateMediaItem(mediaItem) {
  if (!mediaItem.title) return "Title is required.";
  if (!mediaItem.directorId) return "Director is required.";
  if (!mediaItem.format) return "Format is required.";
  return "";
}

function validateDirector(director) {
  if (!director.name) return "Name is required.";
  return "";
}
